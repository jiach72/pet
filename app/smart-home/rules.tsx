...
export default function RulesScreen() {
    ...
    const handleToggle = (ruleId: string) => {
        const newState = toggleRule(ruleId);
        setRules(getRules());
    };
            ...
            {rules.length === 0 ? (
                <View style={styles.emptyState}>
                    <View style={styles.emptyIcon}>
                        <Ionicons name="git-branch" size={48} color={colors.muted} />
                    </View>
                    <Text style={styles.emptyTitle}>暂无自动化规则</Text>
                    <Text style={styles.emptyDesc}>
                        通过快捷场景一键创建，或手动创建自定义规则
                    </Text>
                </View>
            ) : (
                            ...
                            <View key={rule.id} style={styles.ruleCard}>
                                <View style={styles.ruleHeader}>
                                    <View style={styles.ruleNameRow}>
                                        <Ionicons
                                            name={rule.isActive ? "radio-button-on" : "radio-button-off"}
                                            size={20}
                                            color={rule.isActive ? colors.secondary : colors.muted}
                                        />
                                ...
                                <View style={styles.ruleFlow}>
                                    ...
                                    <View style={styles.flowItem}>
                                        ...
                                        >
                                            <Ionicons name="cog" size={16} color={colors.secondary} />
                                        </View>
                                        <Text style={styles.flowLabel}>则</Text>
                                        <Text style={styles.flowValue}>
                                            {rule.action.deviceName} {rule.action.description}
                                        </Text>
                                    </View>
                                </View>
